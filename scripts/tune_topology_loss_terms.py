...
def main() -> None:
    ...
    for idx, value in enumerate(_parse_values(args.lambda_d_grid)):
        tag = f"{base_tag}_landscape_{idx}"
        cmd = [
            PYTHON_EXE,
        ...
        run_command(cmd, f"landscape_lambda_d={value}")
        pred_path = output_dir / f"tgn_landscape_constraint_predictions_{tag}.csv"
        rmse = evaluate_rmse(pred_path)
        rows.append({"dataset": args.dataset, "model": "Landscape", "lambda_value": value, "rmse": rmse, "tag": tag})
