    ...
    for step in tqdm(steps, desc=f"{args.dataset} pipeline", unit="step"):
        if step == "cache_windows":
            ...
            if feature_npz.exists() and table_csv.exists() and not args.force_rebuild:
                ...
                build_cmd = [
                    PYTHON_EXE,
                    "scripts/build_infectious_pilot_table.py",
                    "--dataset-key",
                    cfg["dataset_key"],
                    "--window-seconds",
        ...
        elif step == "train_landscape":
            train_and_cache_model(
                dataset_name=args.dataset,
                model_name="Landscape",
                base_cmd=[PYTHON_EXE, "scripts/train_tgn_landscape_constraint.py"],
                cfg=cfg,
                feature_npz=feature_npz,
                preprocessed_cache_dir=cache_dir,
                epochs=args.epochs,
...
def train_and_cache_model(
    ...
    if model_name == "TGN":
        ...
        pred_path = output_dir / f"tgn_landscape_constraint_predictions_{tag}.csv"
    else:
...
def update_collective_csvs(dataset_name: str, table_csv: Path, output_dir: Path) -> None:
    model_files = {
        ...
        "Landscape": output_dir / f"tgn_landscape_constraint_predictions_{dataset_name.lower()}_landscape.csv",
...
def reset_dataset_cache(dataset_name: str) -> None:
    ...
    output_patterns = [
        ...
        f"tgn_landscape_constraint_history_{slug}_landscape.csv",
        f"tgn_landscape_constraint_predictions_{slug}_landscape.csv",
    ...
    weight_patterns = [
        ...
        f"tgn_landscape_constraint_{slug}_landscape.pt",
