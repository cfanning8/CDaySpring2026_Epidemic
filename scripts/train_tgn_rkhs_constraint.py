...
def main() -> None:
    ...
    fusion_head = nn.Sequential(
        nn.Linear(args.memory_dim, args.fusion_hidden_dim),
        nn.ReLU(),
        nn.Linear(args.fusion_hidden_dim, 1),
        nn.Sigmoid(),
    ).to(device)
    params = list(tgn.parameters()) + list(latent_to_rkhs.parameters()) + list(fusion_head.parameters())
